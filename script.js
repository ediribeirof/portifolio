...
function showCustomAlert(message) {
    if (customAlert && customAlertMessage && customAlertClose) {
        ...
        const timeout = setTimeout(() => {
            ...
            clearTimeout(timeout); // Cancela o timeout após execução
...
if (contactForm) {
    ...
    contactForm.addEventListener('submit', (e) => {
        ...
        if (validateForm(formData)) {
            ...
            // Formata o número do WhatsApp com máscara (85) 99999-9999
            const formattedWhatsapp = `(${whatsapp.substring(0, 2)}) ${whatsapp.substring(2, 7)}-${whatsapp.substring(7, 11)}`;

            // Mapeia o valor de videoType para a frase completa
            const videoTypeMessage = videoType === 'Mesmo' 
                ? 'Os vídeos serão: O mesmo para Instagram e TikTok' 
                : 'Os vídeos serão: Roteiros diferentes (um para cada plataforma)';

            // Mapeia o valor de presence para a frase completa
            let presenceMessage;
            if (presence === 'Loja') {
                presenceMessage = 'Presença física: Presença da Edi em loja para gravação no local';
            } else if (presence === 'Casa') {
                presenceMessage = 'Presença física: Gravação na casa da Edi';
            } else {
                presenceMessage = `Presença física (outra opção): ${presenceOther}`;
            }

            // Formata a mensagem para o WhatsApp com quebras de linha e marcadores
            const whatsappMessage = `Olá! Tudo bem? Tenho interesse em fechar uma parceria de publicidade com a Edi. Seguem as informações do briefing:%0A` +
                                   `• Nome da Marca/Empresa: ${encodeURIComponent(brand)}%0A` +
                                   `• Responsável: ${encodeURIComponent(responsible)}%0A` +
                                   `• WhatsApp: ${encodeURIComponent(formattedWhatsapp)}%0A` +
                                   ...
                                   `• ${encodeURIComponent(videoTypeMessage)}%0A` +
                                   `• Combos de Stories: ${encodeURIComponent(stories)}%0A` +
                                   `• ${encodeURIComponent(presenceMessage)}%0A` +
